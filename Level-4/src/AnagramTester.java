...
public class AnagramTester {
	...
	public boolean isItAnAnagram() {
		/*
		 * Checks if it is an anagram, returns true if it is and false if it is
		 * not 1) Turns them in to arrays 2)checks if the length is diffrent 3)
		 * Sorts the arrays 4) Compars the arrays if they are the same
		 */
	...
	public void setWords() {
		/*
		 * Asks for user input Does one after the other
		 */
	...
	public void printFinish(boolean result) {
		/*
		 * Prints out if it is an anagram or not The first is if it is, the
		 * second one if it is not
		 */
		if (result) {
